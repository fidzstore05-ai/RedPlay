...
<div class="admin-wrap">
  ...
  <aside class="admin-sidebar">
    ...
    <ul class="sidebar-menu">
      ...
      <li>
        <a href="{{ route('genres.manage') }}" class="active">
  ...
  <div class="admin-main">
    ...
    <div class="admin-topbar">
      ...
      <a href="{{ route('genres.create') }}" class="btn-add">
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          @forelse($genres as $g)
          <tr>
            <td style="color:rgba(255,255,255,.3);font-size:.8rem;">{{ $g->id_genre }}</td>
            <td style="font-weight:600;">{{ $g->genre }}</td>
            <td style="color:rgba(255,255,255,.5);">{{ $g->films_count ?? 0 }} Film</td>
            <td>
              <div class="action-btns">
                <a href="{{ route('genres.edit', $g->id_genre) }}" class="btn-edit"> Edit</a>
                <form action="{{ route('genres.destroy', $g->id_genre) }}" method="POST" onsubmit="return confirm('Hapus genre ini?')">
                  @csrf @method('DELETE')
          ...
          @empty
          <tr>
            <td colspan="4" style="text-align:center;padding:3rem;color:rgba(255,255,255,.3);">Tidak ada genre ditemukan</td>
          </tr>
          @endforelse
        </tbody>
      </table>
      @if(method_exists($genres, 'links'))
        <div class="pagination-wrap">{{ $genres->links() }}</div>
      @endif
    </div>
